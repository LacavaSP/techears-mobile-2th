...
const FuncionariosForm: React.FC = () => {  
  ...
  async function obterDadosDoFuncionario() {
    try {
      const response = await axios.get('http://192.168.0.10:1880/api/funcionario/' + id) 
  ...
  async function botaoSalvar() {
    ...
    try {
      if (id) {
        await axios.put('http://192.168.0.10:1880/api/funcionarios/editar', objeto)
      } else {
        await axios.post('http://192.168.0.10:1880/api/funcionarios/criar', objeto)
      }
